...
NSString *const kPTAccountReceivesInAppNotifications =
...
- (NSURL *)baseURL {
  NSURL *baseURL =
      [NSURL URLWithString:@"https://www.pivotaltracker.com/services/v5/"];
...
- (AFHTTPSessionManager *)manager {
  ...
  [manager.requestSerializer setValue:self.apiKey
                   forHTTPHeaderField:@"X-TrackerToken"];
  return manager;
}

- (void)me:(PTAccountSuccess)success failure:(PTAccountFailure)failure {
...
- (void)meold:(void (^)(BOOL, PTProjects *_Nullable))success
...
- (void)getProjects:(void (^)(BOOL, PTProjects *_Nullable))success
            failure:(void (^)(NSError *_Nullable))failure {
}
...
- (instancetype)initWithDictionary:(NSDictionary *)dict {
  ...
  if (self && [dict isKindOfClass:[NSDictionary class]]) {
    self.internalBaseClassIdentifier =
        [[self objectOrNilForKey:kPTAccountId fromDictionary:dict] doubleValue];
    self.timeZone = [PTTimeZone
        modelObjectWithDictionary:[dict objectForKey:kPTAccountTimeZone]];
    ...
    if ([receivedPTProjects isKindOfClass:[NSArray class]]) {
      for (NSDictionary *item in (NSArray *)receivedPTProjects) {
        if ([item isKindOfClass:[NSDictionary class]]) {
          [parsedPTProjects
              addObject:[PTProject modelObjectWithDictionary:item]];
        }
      }
    } else if ([receivedPTProjects isKindOfClass:[NSDictionary class]]) {
      [parsedPTProjects
          addObject:[PTProject
                        modelObjectWithDictionary:(NSDictionary *)
                                                      receivedPTProjects]];
    }

    self.projects = [NSArray arrayWithArray:parsedPTProjects];
    self.createdAt =
        [self objectOrNilForKey:kPTAccountCreatedAt fromDictionary:dict];
    self.receivesInAppNotifications =
        [[self objectOrNilForKey:kPTAccountReceivesInAppNotifications
                  fromDictionary:dict] boolValue];
    self.apiToken =
        [self objectOrNilForKey:kPTAccountApiToken fromDictionary:dict];
    self.updatedAt =
        [self objectOrNilForKey:kPTAccountUpdatedAt fromDictionary:dict];
    self.username =
        [self objectOrNilForKey:kPTAccountUsername fromDictionary:dict];
    self.hasGoogleIdentity =
        [[self objectOrNilForKey:kPTAccountHasGoogleIdentity
                  fromDictionary:dict] boolValue];
    ...
    self.initials =
        [self objectOrNilForKey:kPTAccountInitials fromDictionary:dict];
...
- (NSDictionary *)dictionaryRepresentation {
  ...
  [mutableDict setValue:[NSArray arrayWithArray:tempArrayForProjects]
                 forKey:kPTAccountProjects];
  [mutableDict setValue:self.createdAt forKey:kPTAccountCreatedAt];
  [mutableDict
      setValue:[NSNumber numberWithBool:self.receivesInAppNotifications]
  ...
  [mutableDict setValue:[NSNumber numberWithBool:self.hasGoogleIdentity]
                 forKey:kPTAccountHasGoogleIdentity];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  self.receivesInAppNotifications =
      [aDecoder decodeBoolForKey:kPTAccountReceivesInAppNotifications];
  ...
  self.hasGoogleIdentity =
      [aDecoder decodeBoolForKey:kPTAccountHasGoogleIdentity];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  ...
  [aCoder encodeBool:_receivesInAppNotifications
              forKey:kPTAccountReceivesInAppNotifications];
...
- (void)XPCSuccess:(id)object {
}

- (void)XPCFailure:(NSError *)error {
}
@end
