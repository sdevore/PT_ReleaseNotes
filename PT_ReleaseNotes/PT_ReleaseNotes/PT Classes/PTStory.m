...
- (instancetype)initWithDictionary:(NSDictionary *)dict {
    ...
    if (self && [dict isKindOfClass:[NSDictionary class]]) {
        self.internalBaseClassIdentifier = [[self objectOrNilForKey:kPTStoryId fromDictionary:dict] doubleValue];
        self.internalBaseClassDescription = [self objectOrNilForKey:kPTStoryDescription fromDictionary:dict];
        self.createdAt = [self objectOrNilForKey:kPTStoryCreatedAt fromDictionary:dict];
        self.labels = [self objectOrNilForKey:kPTStoryLabels fromDictionary:dict];
        self.acceptedAt = [self objectOrNilForKey:kPTStoryAcceptedAt fromDictionary:dict];
        self.estimate = [[self objectOrNilForKey:kPTStoryEstimate fromDictionary:dict] doubleValue];
        self.url = [self objectOrNilForKey:kPTStoryUrl fromDictionary:dict];
        self.currentState = [self objectOrNilForKey:kPTStoryCurrentState fromDictionary:dict];
        self.ownedById = [[self objectOrNilForKey:kPTStoryOwnedById fromDictionary:dict] doubleValue];
        self.storyType = [self objectOrNilForKey:kPTStoryStoryType fromDictionary:dict];
        self.requestedById = [[self objectOrNilForKey:kPTStoryRequestedById fromDictionary:dict] doubleValue];
        self.updatedAt = [self objectOrNilForKey:kPTStoryUpdatedAt fromDictionary:dict];
        self.projectId = [[self objectOrNilForKey:kPTStoryProjectId fromDictionary:dict] doubleValue];
        self.kind = [self objectOrNilForKey:kPTStoryKind fromDictionary:dict];
        self.ownerIds = [self objectOrNilForKey:kPTStoryOwnerIds fromDictionary:dict];
        self.name = [self objectOrNilForKey:kPTStoryName fromDictionary:dict];
        self.deadline = [self objectOrNilForKey:kPTStoryDeadline fromDictionary:dict];
    }
...
- (NSDictionary *)dictionaryRepresentation {
    ...
    [mutableDict setValue:[NSNumber numberWithDouble:self.internalBaseClassIdentifier] forKey:kPTStoryId];
    [mutableDict setValue:self.internalBaseClassDescription forKey:kPTStoryDescription];
    ...
    [mutableDict setValue:[NSArray arrayWithArray:tempArrayForLabels] forKey:kPTStoryLabels];
    [mutableDict setValue:self.acceptedAt forKey:kPTStoryAcceptedAt];
    [mutableDict setValue:[NSNumber numberWithDouble:self.estimate] forKey:kPTStoryEstimate];
    [mutableDict setValue:self.url forKey:kPTStoryUrl];
    [mutableDict setValue:self.currentState forKey:kPTStoryCurrentState];
    [mutableDict setValue:[NSNumber numberWithDouble:self.ownedById] forKey:kPTStoryOwnedById];
    [mutableDict setValue:self.storyType forKey:kPTStoryStoryType];
    [mutableDict setValue:[NSNumber numberWithDouble:self.requestedById] forKey:kPTStoryRequestedById];
    [mutableDict setValue:self.updatedAt forKey:kPTStoryUpdatedAt];
    [mutableDict setValue:[NSNumber numberWithDouble:self.projectId] forKey:kPTStoryProjectId];
    ...
    for (NSObject *subArrayObject in self.ownerIds) {
        if ([subArrayObject respondsToSelector:@selector(dictionaryRepresentation)]) {
            // This class is a model object
            [tempArrayForOwnerIds addObject:[subArrayObject performSelector:@selector(dictionaryRepresentation)]];
        } else {
            // Generic object
            [tempArrayForOwnerIds addObject:subArrayObject];
        }
    }
    [mutableDict setValue:[NSArray arrayWithArray:tempArrayForOwnerIds] forKey:kPTStoryOwnerIds];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    self.internalBaseClassDescription = [aDecoder decodeObjectForKey:kPTStoryDescription];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    [aCoder encodeObject:_internalBaseClassDescription forKey:kPTStoryDescription];
